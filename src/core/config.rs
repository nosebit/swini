//! Global configuration loading, merging, and path resolution for Swini.
//!
//! Provides the [`Config`] struct and resolution hierarchy:
...
//!    `--config <path>`).
//! 3. Environment variable overrides (`SWINI_ADDR`).
//!
...
/// Default socket address where the Plot's Clerk listens for incoming Ranch
/// traffic.
pub const DEFAULT_ADDR: &str = "127.0.0.1:7440";

/// Default instance name for the local Regent process on the Plot.
pub const DEFAULT_NAME: &str = "main";

/// Global configuration for a Swini Plot and Regent runtime.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct Config {
  /// Name identifying this Plot / Regent instance.
  #[serde(default = "default_name")]
  pub name: String,

  /// Canonical network address where this Plot's Clerk listens.
  #[serde(default = "default_addr")]
  pub addr: SocketAddr,

  /// Root directory for runtime state, consensus data, and rolling logs.
  #[serde(default)]
  pub data_dir: PathBuf,

  /// Operational roles assigned to this Plot.
  #[serde(default = "default_roles")]
  pub roles: Vec<PlotRole>,

  /// User-defined tags associated with this Plot.
  #[serde(default)]
  pub tags: Vec<String>,
...
/// Computes the directory housing all Regents on this host: `~/.swini/regents`.
pub fn regents_dir() -> PathBuf {
  let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
  PathBuf::from(home).join(".swini").join("regents")
}

/// Computes the root data directory for a named Regent:
/// `~/.swini/regents/{name}`.
pub fn default_data_dir(name: &str) -> PathBuf {
  regents_dir().join(name)
}
...
mod tests {
  ...
  fn default_config_values() {
    ...
    assert_eq!(config.roles, vec![PlotRole::Server, PlotRole::Worker]);
    assert!(config.tags.is_empty());
    assert!(config.join_addresses.is_empty());
    assert!(config.data_dir.ends_with(".swini/regents/main"));
  }
...
"#;
    ...
    assert_eq!(config.roles, vec![PlotRole::Worker]);
    assert_eq!(config.tags, vec!["gpu", "fast-storage"]);
    assert_eq!(config.join_addresses, vec!["127.0.0.1:7440"]);
    assert!(config.data_dir.ends_with(".swini/regents/worker-node"));
  }
